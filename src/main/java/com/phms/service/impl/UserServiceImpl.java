...
public class UserServiceImpl implements UserService {
	...
	@Override
	public ResultMap githubLogin(String username, String password) {
		// 从SecurityUtils里边创建一个 subject
		Subject subject = SecurityUtils.getSubject();
		// 在认证提交前准备 token（令牌）
...
//		return userMapper.selectByExample(example);
		List<User> docs = new LinkedList<>();
		UserRoleExample userRole = new UserRoleExample();
		userRole.createCriteria().andRoleIdEqualTo(3);
		List<UserRole> userRoles = userRoleMapper.selectByExample(userRole);
		for (UserRole userRole1: userRoles) {
			User user = userMapper.selectByPrimaryKey(Long.parseLong(userRole1.getUserId()));
			docs.add(user);
		}
		return docs;
	}
