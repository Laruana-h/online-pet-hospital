...
public class HealthController{
    ...
    public String applyListDoctor(Model model) {
        ...
        for (Pet p : rows) {
            ...
            switch (sw) {
                case 1:
                    dsCount.add(p.getName() + "  Pet health, please continue to keep");
                    break;
                case 2:
                    dsCount.add(p.getName() + "  Pets abnormal,please see a doctor in time!");
                    break;
                case 3:
                    dsCount.add(p.getName() + "  Pets are seriously ill, please seek medical attention in time!");
                    break;
                case 4:
                    dsCount.add(p.getName() + "  I'm sorry the pet can't be treated!");
                    break;
                default:
                    dsCount.add(p.getName() + "  Pet is basically healthy, please keep it");
                    break;
            }
            ...
            if (t >= tsMin && t <= tsMax) {
                tsCount.add(" Normal animal heat");
            } else if (t < tsMin) {
                tsCount.add(" Low animal heat");
            } else if (t > tsMax) {
                tsCount.add(" High animal heat");
            }

            if (w >= wsMin && w <= wsMax) {
                wsCount.add(" Normal weight");
            } else if (w < wsMin) {
                wsCount.add(" Low weight");
            } else if (w > wsMax) {
                wsCount.add(" High weight");
            }

            if (h >= hsMin && h <= hsMax) {
                hsCount.add(" Normal height");
            } else if (h < hsMin) {
                hsCount.add(" Low height");
            } else if (h > hsMax) {
                hsCount.add(" High height");
            }

            if (a >= asMin && a <= asMax) {
                asCount.add(" Normal appetite");
            } else if (a < asMin) {
                asCount.add("  Low appetite");
            } else if (a > asMax) {
                asCount.add(" High appetite");
            }
        }
        model.addAttribute("pets", rows);
